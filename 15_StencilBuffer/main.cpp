...
int main() {
  ...
  std::string outlinePath = "/crow-outline/scene.gltf";
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // glUniform1f(glGetUniformLocation(outliningProgram.ID,
    // "outlining"), 1.08f); model.Draw(outliningProgram, camera);

    // Second method from the tutorial
    // outliningProgram.Activate();
    // glUniform1f(glGetUniformLocation(outliningProgram.ID, "outlining"),
    // 0.08f); model.Draw(outliningProgram, camera);
