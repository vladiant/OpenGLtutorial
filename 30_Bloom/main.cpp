...
std::vector<Vertex> vertices = {
    ...
    Vertex{glm::vec3(1.0f, -1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f),
           glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(1.0f, 0.0f)}};
...
int main() {
  ...
  Texture displacementMap((parentDir + displacementPath).c_str(),
                          "displacement", 2);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (unsigned int i = 0; i < amount; i++) {
      ...
      glUniform1i(glGetUniformLocation(blurProgram.ID, "horizontal"),
                  horizontal);
