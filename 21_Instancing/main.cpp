...
int main() {
  ...
  glUniform4f(glGetUniformLocation(asteroidShader.ID, "lightColor"),
              lightColor.x, lightColor.y, lightColor.z, lightColor.w);
  glUniform3f(glGetUniformLocation(asteroidShader.ID, "lightPos"), lightPos.x,
              lightPos.y, lightPos.z);
  ...
  std::string jupiterPath = "/jupiter/scene.gltf";
  std::string asteroidPath = "/asteroid/scene.gltf";
  ...
  std::string facesCubemap[6] = {
      parentDir + "/skybox/right.png", parentDir + "/skybox/left.png",
      parentDir + "/skybox/top.png",   parentDir + "/skybox/bottom.png",
      parentDir + "/skybox/front.png", parentDir + "/skybox/back.png",
  };
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // We make the mat4 into a mat3 and then a mat4 again in order to get rid of
    // the last row and column The last row and column affect the translation of
    // the skybox (which we don't want to affect)
    view = glm::mat4(glm::mat3(glm::lookAt(
        camera.Position, camera.Position + camera.Orientation, camera.Up)));
    projection = glm::perspective(glm::radians(45.0f), (float)width / height,
                                  0.1f, 100.0f);
    ...
    // Draws the cubemap as the last object so we can save a bit of performance
    // by discarding all fragments where an object is present (a depth of 1.0f
    // will always fail against any object's depth value)
    glBindVertexArray(skyboxVAO);
